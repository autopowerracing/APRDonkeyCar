...
download_simulation() {
  if [ ! -d "./Simulator-Binaries" ]; then
    ...
    downloadUrls=$(curl -s "https://api.github.com/repos/tawnkramer/gym-donkeycar/releases/latest" | grep "browser_download")
    ...
    wget $binaryUrl -P ./Simulator-Binaries/ --quiet --show-progress
...
main() {
  ...
  __bs_print_title 1 5 "Installing Prerequisites"
  ...
  __bs_print_title 2 5 "Installing DonkeyCar"
  ...
  __bs_print_title 3 5 "Installing GYM Donkeycar"
  ...
  __bs_print_title 4 5 "Creating Initial Donkey Car Application"
  if __bs_run_func_with_margin install_gymdonkeycar; then
    __bs_print_bare_footer
  else
    __bs_print_fail_footer
   ...
   __bs_print_title 5 5 "Downloading Simulator Binaries"
